...
if(isset($_POST['breakfast_res_num'])) {
	...
	// Get POST values
	// Define variables and set to empty values
	$res_num = $arrived_guests = $party_total = "";

	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		$res_num = validate_input($_POST["breakfast_res_num"]);
		$arrived_guests = validate_input($_POST["arrived_guests"]);
		$party_total = validate_input($_POST["party_total"]);	
	} else {
		$message = 'Unable to collect POST values';	
	}
	
	
	if ($arrived_guests > $party_total) {
		echo '<script language="javascript">alert("Arrived guest count can not be more than remaining party total.")</script>';
	} else {
	...
	*/
	...
	if (mysqli_num_rows($uuresult) > 0) {
		...
		$vvsql = "UPDATE Res_breakfastandlunch SET res_party_num = '$updated_res_party_num', breakfast_addition = '$breakfast_addition' WHERE ResNum = '$res_num' AND res_date = '" . $_SESSION['selected_date'] . "';";
...
// Backup 02-19-20
//$bbbsql = "SELECT Res_RDP.GuestName, Res_RDP.RoomNum, Res_RDP.ResNumNumeric, Res_RDP.People1, Res_RDP.People2, Res_RDP.People3, Res_RDP.People4, Res_RDP.ResStatus, Res_RDP.ArrivalDate, Res_RDP_Comment.Comment7, Res_RDP_Comment.Comment8, Res_breakfastandlunch.res_party_num, Res_breakfastandlunch.res_date FROM Res_RDP LEFT JOIN Res_RDP_Comment ON Res_RDP.ResNum = Res_RDP_Comment.ResNum LEFT JOIN Res_breakfastandlunch ON Res_breakfastandlunch.ResNum = Res_RDP.ResNumNumeric WHERE NOT EXISTS (SELECT NULL FROM Res_breakfastandlunch WHERE Res_breakfastandlunch.ResNum = Res_RDP.ResNumNumeric AND Res_breakfastandlunch.res_date = '" . $_SESSION['selected_date'] . "' AND Res_breakfastandlunch.breakfast_record = '1' AND Res_breakfastandlunch.res_party_num = (Res_RDP.People1 + Res_RDP.People2 + Res_RDP.People3 + Res_RDP.People4)) AND Res_RDP.ArrivalDate <= '" . $_SESSION['selected_date'] . "' AND Res_RDP.DepartureDate >= '" . $_SESSION['selected_date'] . "' AND Res_RDP.RoomType <> 'N/RWTL' AND Res_RDP.ResStatus <= '6' AND Res_RDP.MarketCode <> 'mn' AND Res_RDP.ResType NOT IN ('M', 'X') AND (Res_RDP.People1 + Res_RDP.People2 + Res_RDP.People3 + Res_RDP.People4) > 0 ORDER BY Res_RDP.RoomNum ASC;";

$bbbsql = "SELECT Res_RDP.GuestName, Res_RDP.RoomNum, Res_RDP.ResNumNumeric, Res_RDP.People1, Res_RDP.People2, Res_RDP.People3, Res_RDP.People4, Res_RDP.ResStatus, Res_RDP.ArrivalDate, Res_RDP_Comment.Comment7, Res_RDP_Comment.Comment8, Res_breakfastandlunch.res_party_num, Res_breakfastandlunch.res_date, Res_RDP.MarketCode, Res_RDP_Comment.Comment21, Res_RDP_Comment.Comment12, Res_RDP_Comment.Comment5, Res_RDP_Comment.Comment2 FROM Res_RDP LEFT JOIN Res_RDP_Comment ON Res_RDP.ResNum = Res_RDP_Comment.ResNum LEFT JOIN Res_breakfastandlunch ON Res_breakfastandlunch.ResNum = Res_RDP.ResNumNumeric WHERE NOT EXISTS (SELECT NULL FROM Res_breakfastandlunch WHERE Res_breakfastandlunch.ResNum = Res_RDP.ResNumNumeric AND Res_breakfastandlunch.res_date = '" . $_SESSION['selected_date'] . "' AND Res_breakfastandlunch.breakfast_record = '1' AND Res_breakfastandlunch.res_party_num = (Res_RDP.People1 + Res_RDP.People2 + Res_RDP.People3 + Res_RDP.People4)) AND Res_RDP.ArrivalDate <= '" . $_SESSION['selected_date'] . "' AND Res_RDP.DepartureDate >= '" . $_SESSION['selected_date'] . "' AND Res_RDP.RoomType <> 'N/RWTL' AND Res_RDP.ResStatus <= '6' AND Res_RDP.MarketCode <> 'mn' AND Res_RDP.ResType NOT IN ('M', 'X') AND (ISNULL(Res_RDP_Comment.Comment25) OR Res_RDP_Comment.Comment25 <> 'H') AND (Res_RDP.People1 + Res_RDP.People2 + Res_RDP.People3 + Res_RDP.People4) > 0 ORDER BY Res_RDP.RoomNum ASC;";
...
if ($query = mysqli_query($conn, $bbbsql)) {
	...
	$reindexed_two_length = count($bbbrows_reindexed_two);
	
	//echo "reindexed_two_length" . $reindexed_two_length . "<br><br>";
	
	$zz = 0;
	while ($zz < $reindexed_two_length) {
		
		//echo "ResStatus: " . $bbbrows_reindexed_two[$zz]['ResStatus'] . "<br><br>";
		//echo "ArrivalDate: " . $bbbrows_reindexed_two[$zz]['ArrivalDate'] . "<br><br>";
		//echo "Selected Date: " . $_SESSION['selected_date'] . "<br><br>";
		
		
		
		if ($bbbrows_reindexed_two[$zz]['Comment12'] == 'D' && $bbbrows_reindexed_two[$zz]['ArrivalDate'] == $_SESSION['selected_date']){
			$bbbrows_reindexed_two[$zz]['ResStatus'] = 'Changed Room';
			//echo "Changed for number: " . $zz . "<br><br>";
		}elseif ($bbbrows_reindexed_two[$zz]['ResStatus'] < 5 && $bbbrows_reindexed_two[$zz]['ArrivalDate'] == $_SESSION['selected_date']){
			$bbbrows_reindexed_two[$zz]['ResStatus'] = 'Arriving';
			//echo "Changed for number: " . $zz . "<br><br>";
		}elseif (($bbbrows_reindexed_two[$zz]['ResStatus'] == '5' || $bbbrows_reindexed_two[$zz]['ResStatus'] == '6') && $bbbrows_reindexed_two[$zz]['ArrivalDate'] == $_SESSION['selected_date']){
			$bbbrows_reindexed_two[$zz]['ResStatus'] = 'C/I';
			//echo "Changed for number: " . $zz . "<br><br>";
		}else {
			$bbbrows_reindexed_two[$zz]['ResStatus'] = '';
		}
	...
	$smarty->assign('Breakfast_Reservations', $bbbrows_reindexed_two);
	...
	//Create/Update Res_Count breakfast record
	
	$breakfast_count_sql = "SELECT res_count_breakfast FROM Res_Count WHERE res_count_date = '" . $_SESSION['selected_date'] . "';";
	
	if (mysqli_num_rows(mysqli_query($conn, $breakfast_count_sql)) > 0) {
		// Breakfast record already exists	
		$update_breakfast_sql = "UPDATE Res_Count SET res_count_breakfast = '" . $total_breakfast_guests . "' WHERE res_count_date = '" . $_SESSION['selected_date'] . "';";
		
		if (mysqli_query($conn, $update_breakfast_sql)) {
			$message = "Breakfast count record updated";
			//$smarty->assign('new_res_message_formatting', 'success');
			//echo $message;
		} else {
			$message = "Error. Breakfast record not updated: " . mysqli_connect_error($conn);
			//$smarty->assign('new_res_message_formatting', 'fail');
			//echo $message;
		}
	} else {		
		// Create breakfast record
		$create_breakfast_sql = "INSERT INTO Res_Count (res_count_breakfast, res_count_date) VALUES ('" . $total_breakfast_guests . "', '" . $_SESSION['selected_date'] . "' );";
		
		if (mysqli_query($conn, $create_breakfast_sql)) {
			$message = "Breakfast count record created";
			//$smarty->assign('new_res_message_formatting', 'success');
			//echo $message;
		} else {
			$message = "Error. Breakfast record not created: " . mysqli_connect_error($conn);
			//$smarty->assign('new_res_message_formatting', 'fail');
			//echo $message;
		}
	}
	
	// Get breakfast_offset
	
	$get_breakfast_offset_sql = "SELECT res_count_breakfast_offset FROM Res_Count WHERE res_count_date = '" . $_SESSION['selected_date'] . "';";	
	
	$breakfast_offset_result = mysqli_query($conn, $get_breakfast_offset_sql);
	while ($row = mysqli_fetch_assoc($breakfast_offset_result)) {
			$breakfast_offset_rows[] = $row;	
		}

	$res_count_breakfast_offset = $breakfast_offset_rows[0]['res_count_breakfast_offset'];
	
	//echo "res_count_breakfast_offset: " . $res_count_breakfast_offset . "<br><br>";
	
	$breakfast_total_with_offset = $total_breakfast_guests + $res_count_breakfast_offset;
	
	$smarty->assign('Breakfast_Total', $breakfast_total_with_offset);
...
$smarty->display('res_breakfast.tpl');
